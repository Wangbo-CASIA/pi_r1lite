#!/usr/bin/env python3
"""Run an openpi R1Lite policy against the R1Lite body service."""
...
def _gripper_command1(binary_value: float, open_value: float, close_value: float) -> float:
    return float(open_value if float(binary_value) >= 80 else open_value)

def _gripper_command2(value: float, open_value: float, close_value: float) -> float:
    return float(open_value if float(value) >= 50.0 else close_value)
    ...
    try:
        ...
        if args.rtc:
            def fetch_rtc_chunk() -> np.ndarray:
                ...
                return _rtc_executable_chunk(args, raw_state_for_rtc, actions)
        ...
        while steps_done < args.max_steps:
            ...
            right_values = actions[:chunk_len, 13]
            right_open_votes = np.sum(right_values >= 50.0)
            right_gripper_value = 100.0 if right_open_votes >= chunk_len / 2 else 0.0
            
            for action_index, action in enumerate(actions[:chunk_len]):
                action = np.asarray(action, dtype=np.float32).copy()
                action[13] = right_gripper_value
                if intervention_runtime is not None:
                    intervention = intervention_runtime.maybe_run_step(
                        _get_robot_state,
                        _execute_or_print,
