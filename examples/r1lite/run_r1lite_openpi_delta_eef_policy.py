#!/usr/bin/env python3
"""Run a relative-EEF openpi R1Lite policy against the R1Lite body service."""
...
def _target_pose_from_delta(reference_pose: np.ndarray, delta: np.ndarray, label: str) -> np.ndarray:
    reference_pose = _unit_quat(reference_pose, f"{label} reference pose")
    delta = np.asarray(delta, dtype=np.float32).reshape(-1)
    if delta.shape[0] < 6:
        raise ValueError(f"{label} delta must contain at least 6 values, got {delta.shape[0]}")
    pose = reference_pose.copy()
    pose[:3] = pose[:3] + delta[:3]
    pose[3:7] = (Rotation.from_euler("xyz", delta[3:6]) * Rotation.from_quat(reference_pose[3:7])).as_quat()
    return _unit_quat(pose, f"{label} target pose")
...
def _payload_from_action(
    ...
    left_target = _target_pose_from_delta(left_reference, action[:6], "left")
    right_target = _target_pose_from_delta(right_reference, action[7:13], "right")
    left_gripper = _gripper_command(action[6], gripper_open_value, gripper_close_value)
    right_gripper = _gripper_command(action[13], gripper_open_value, gripper_close_value)
    left_payload = {
    ...
    return payload, StepRecord(
        seq=seq,
        left_reference=left_reference.copy(),
        right_reference=right_reference.copy(),
...
def _rtc_executable_chunk(args: argparse.Namespace, raw_state: dict[str, Any], actions: np.ndarray) -> np.ndarray:
    left_reference = _tcp_pose(raw_state, "left")
    right_reference = _tcp_pose(raw_state, "right")
    chunk_len = min(args.actions_per_infer, actions.shape[0])
    rows = []
    for raw_action in actions[:chunk_len]:
        action = np.asarray(raw_action, dtype=np.float32).reshape(-1)
        if action.shape[0] != ACTION_DIM:
            raise ValueError(f"Expected one action with {ACTION_DIM} values, got {action.shape[0]}")
        left_target = _target_pose_from_delta(left_reference, action[:6], "left")
        right_target = _target_pose_from_delta(right_reference, action[7:13], "right")
        rows.append(np.concatenate([left_target, action[6:7], right_target, action[13:14]], axis=0))
        left_reference = left_target
        right_reference = right_target
    return np.asarray(rows, dtype=np.float32)


def _payload_from_absolute_pose_action(
    action: np.ndarray,
    seq: int,
    gripper_open_value: float,
    gripper_close_value: float,
    ...
    if action.shape[0] != 16:
        raise ValueError(f"Expected one RTC absolute EEF action with 16 values, got {action.shape[0]}")
    left_target = _unit_quat(action[:7], "left RTC action pose")
    right_target = _unit_quat(action[8:15], "right RTC action pose")
...
def _next_rtc_action_record(args: argparse.Namespace, action: np.ndarray, seq: int) -> StepRecord:
    payload, record = _payload_from_absolute_pose_action(
        action,
        seq,
        args.gripper_open_value,
        args.gripper_close_value,
...
def _run_debug_chunk(args: argparse.Namespace, actions: np.ndarray, steps_done: int, seq: int) -> tuple[int, int, bool]:
    ...
    while action_index < chunk_len:
        ...
        left_reference = last_record.left_target
        right_reference = last_record.right_target
        steps_done += 1
        seq += 1
        action_index += 1
        if args.execute:
    ...
    try:
        ...
        while steps_done < args.max_steps:
            ...
            for action_index, action in enumerate(actions[:chunk_len]):
                ...
                left_reference = record.left_target
                right_reference = record.right_target
                steps_done += 1
                seq += 1
                if args.execute:
                    time.sleep(1.0 / args.control_hz)
