#!/usr/bin/env python3
"""Convert R1Lite dual-arm RAW MCAP episodes to a relative-EEF LeRobot dataset."""
# ruff: noqa: E402, I001
...
ACTION_NAMES = (
    [f"left_eef_delta_{name}" for name in ("x", "y", "z", "roll", "pitch", "yaw")]
    + ["left_gripper01_target"]
    + [f"right_eef_delta_{name}" for name in ("x", "y", "z", "roll", "pitch", "yaw")]
...
def _euler_left_delta_xyz(current_pose: np.ndarray, next_pose: np.ndarray) -> np.ndarray:
    current_pose = _unit_quat(current_pose, "current tcp pose")
    next_pose = _unit_quat(next_pose, "next tcp pose")
    current = Rotation.from_quat(current_pose[3:7])
    nxt = Rotation.from_quat(next_pose[3:7])
...
def _arm_action(
    current_pose: np.ndarray, next_pose: np.ndarray, next_gripper: np.ndarray, threshold: float
) -> np.ndarray:
    current_pose = _unit_quat(current_pose, "current action pose")
    next_pose = _unit_quat(next_pose, "next action pose")
    return np.concatenate(
        [
            next_pose[:3] - current_pose[:3],
            _euler_left_delta_xyz(current_pose, next_pose),
            _binary_gripper(next_gripper, threshold),
        ],
        axis=0,
    ).astype(np.float32)


def _action_vector(current: dict[str, Any], next_sample: dict[str, Any], gripper_threshold: float) -> np.ndarray:
    return np.concatenate(
        [
            _arm_action(
                current["left_tcp_pose"], next_sample["left_tcp_pose"], next_sample["left_gripper"], gripper_threshold
            ),
            _arm_action(
                current["right_tcp_pose"],
                next_sample["right_tcp_pose"],
                next_sample["right_gripper"],
                gripper_threshold,
            ),
        ],
        axis=0,
...
def _add_episode(
    ...
    for idx, sample in enumerate(samples):
        ...
        frame = {
            ...
            "action": _action_vector(sample, next_sample, gripper_threshold),
            "task": task_desc,
        }
